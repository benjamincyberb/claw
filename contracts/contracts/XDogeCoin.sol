 ...
 */
contract XDOGCoin is ERC20, Ownable {
    ...
    constructor() ERC20("XDOG Coin", "XDOG") Ownable(msg.sender) {
    ...
    function mint(address to, uint256 amount) external onlyOwner {
        require(totalSupply() + amount <= MAX_SUPPLY, "XDOG: max supply exceeded, even the printer has limits");
        _mint(to, amount);
    }
    ...
    function faucet() external {
        require(!hasClaimed[msg.sender], "XDOG: already claimed, no double-dipping");
        require(totalSupply() + FAUCET_AMOUNT <= MAX_SUPPLY, "XDOG: faucet dried up");
