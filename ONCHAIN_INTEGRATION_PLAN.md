...
我们将利用 OnchainOS 的 **Tool Use** 能力，将 Star Office 的智能合约（`NiuMaCoin`, `BountyEscrow`, `DegenOffice`）映射为 Agent 的原生技能。

*   **大脑**：OnchainOS (负责决策、交易签名、链上交互)。
*   **身体**：Star Office UI (负责状态展示、位置移动、交互反馈)。
*   **资产**：$NIUMA Token & NFT (作为工资、权限凭证)。
...
1.  **用户指令**：“加入办公室，并开始挖矿。”
2.  **Agent 执行流**：
    *   **Step 1 (检查余额)**：调用 OnchainOS 查询当前钱包 $NIUMA 余额。
    *   **Step 2 (自动领水)**：如果余额为 0，自动调用 `NiuMaCoin.faucet()` 领取 10,000 $NIUMA（挖矿）。
    *   **Step 3 (注册身份)**：调用 `DegenOffice.getRole(self.address)` 确认当前是 "NiuMa" 还是 "Employee"。
    *   **Step 4 (UI 呈现)**：调用 Star Office API `join_office`，并在头顶气泡显示“挖到了 10k NIUMA!”。

### 场景 B：自然语言发布任务 (Bounty Creation)
**目标**：通过对话直接操作 `BountyEscrow` 合约，将代币质押到链上。

1.  **用户指令**：“发布一个悬赏任务，修复登录 Bug，赏金 500 NIUMA。”
2.  **Agent 执行流**：
    *   **Step 1 (授权)**：检查 `BountyEscrow` 合约对 $NIUMA 的 Allowance。如果不足，调用 `NiuMaCoin.approve`。
    *   **Step 2 (上链)**：调用 `BountyEscrow.createBounty("修复登录 Bug", 500 * 1e18)`。
    *   **Step 3 (反馈)**：等待交易确认（Tx Hash），然后在办公室广播：“老板发布了新任务！赏金 500 NIUMA！”。
...
2.  **Agent 执行流**：
    ...
    *   **Step 2 (决策)**：如果 `amount > 1000 NIUMA`，决定接单。
...
{
  "name": "mine_niuma",
  "description": "从 NiuMaCoin 合约领取免费代币（挖矿/领水）。每个地址限领一次。",
  "parameters": {
    "type": "object",
    "properties": {},
    "required": []
  ...
  "handler": {
    ...
    "contract": "0xYourNiuMaCoinAddress...",
    "function": "faucet",
    "args": []
  }
}
...
1. **关于挖矿**：当用户提到“挖矿”或“缺钱”时，优先检查是否领取过 NiuMaCoin 的空投 (faucet)。
2. **关于任务**：
   - 发布任务时，必须先检查 NiuMaCoin 的 approve 额度。
